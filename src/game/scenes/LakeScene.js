...
export default class LakeScene extends Phaser.Scene {
    constructor() {
        super({ key: 'LakeScene' });
        this.player = null;
        this.cursors = null;
    ...
    preload() {
        // Load the lake map
        this.load.tilemapTiledJSON('lake_map', '/map/lake.tmj');
        
        // Load lake tileset images
        this.load.image('ex_terrain', '/assets/ex_terrain.png');
        this.load.image('ex_lake', '/assets/ex_lake.png');
    ...
    create() {
        ...
        // Create the tilemap
        const map = this.make.tilemap({ key: 'lake_map' });
        
        // Add lake tilesets
        const terrainTileset = map.addTilesetImage('ex_terrain', 'ex_terrain');
        const lakeTileset = map.addTilesetImage('ex_lake', 'ex_lake');
        
        // Create layers in the correct order
        const ground = map.createLayer('ground', [terrainTileset, lakeTileset]);
        ground.setDepth(0);
        ...
        if (furnitureLayer) {
            furniture = map.createLayer('furniture', [terrainTileset, lakeTileset]);
        ...
        this.player = this.physics.add.sprite(128, 200, playerKey); // Start position on lake
        ...
        reduxBridge.updateCurrentScene('LakeScene');
        ...
        // Create exit zone (to return to mainland)
        const exitZone = this.add.zone(50, 50, 32, 32);
        ...
        // Add lake-specific quest markers
        this.addLakeQuests();
    ...
    addLakeQuests() {
        // Create lake activity quests
        const lakeQuests = [
            {
                id: 'lake_fish',
                title: 'Memancing Ikan',
                description: 'Memancing ikan segar di danau',
                x: 160,
                y: 120,
                statChanges: { meal: 25, happiness: 15 },
                scoreIncrease: 90
            },
            {
                id: 'lake_meditation',
                title: 'Meditasi Danau',
                description: 'Meditasi dan relaksasi di tepi danau',
                x: 80,
                y: 180,
            ...
            },
            {
                id: 'lake_boat_ride',
                title: 'Naik Perahu',
                description: 'Menikmati pemandangan dari perahu',
                x: 220,
                y: 160,
                statChanges: { happiness: 35, meal: -5 },
                scoreIncrease: 130
            }
        ];

        // Add quest markers to the scene
        lakeQuests.forEach(quest => {
            // Create star marker
            let marker;
            try {
                if (this.textures.exists('star')) {
                    marker = this.add.image(quest.x, quest.y, 'star');
                    marker.setScale(0.5); // Lake appropriate size (smaller)
                } else {
    ...
    update() {
        ...
        // Check for nearby lake quests (only visible ones)
        const nearbyQuests = this.questMarkers.filter(questMarker => {
            ...
            return distance < 35; // Smaller radius for lake (more compact)
        });
        ...
        if (this.nearestQuestId && cursors.quest.isDown && !this.questKeyPressed && !this.isDoingQuest) {
            console.log(`Starting lake quest: ${this.nearestQuestId}`);
